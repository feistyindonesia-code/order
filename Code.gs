...
// QRIS API Configuration (Server-side only - AMAN)
const QRIS_API_KEY = "3a8ad4e906094138a9f6adf8bf920e6907706444bb2546ccbadb3047bb8b09b6";
const QRIS_API_URL = "https://temanqris.com/api/qris/generate";
...
function doGet(e) {
  ...
  else if (action === 'generateQRIS') {
    const amount = parseInt(e.parameter.amount) || 0;
    result = generateQRIS(amount);
  }
...
function getConfig() {
  ...
  return {
    ...
    qris_fee: settings.qris_fee || 1000
  };
}

// ==================================================
// QRIS GENERATION (Server-side - AMAN)
// ==================================================
function generateQRIS(amount) {
  try {
    if (!amount || amount < 1000) {
      return { success: false, error: 'Amount minimal Rp 1.000' };
    }
    
    const settings = getSettings();
    const qrisFee = settings.qris_fee || 1000;
    ...
    const response = UrlFetchApp.fetch(QRIS_API_URL, options);
    const responseCode = response.getResponseCode();
    const responseText = response.getContentText();
    
    logToSheet('QRIS Response', { code: responseCode, body: responseText.substring(0, 500) });
    
    if (responseCode !== 200 && responseCode !== 201) {
      return { 
        success: false, 
        error: 'QRIS API error: ' + responseCode,
    ...
    logToSheet('QRIS Error', err.toString());
    return { 
      success: false, 
